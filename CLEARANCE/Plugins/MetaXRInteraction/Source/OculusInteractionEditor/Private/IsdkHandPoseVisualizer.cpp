    ...
    FPrimitiveDrawInterface* PDI)
{
  ...
  if (ThisHandGrabPose)
  {
    ...
    if (!IsHandGrabPoseValid(ThisHandGrabPose))
    {
    ...
    for (uint8 ChainIndex = 0; ChainIndex < 5; ChainIndex++)
    {
      const FIsdkBoneChain& Chain =
          ThisHandGrabPose->HandPoseData->HandData->BoneChains[ChainIndex];
    ...
    UIsdkHandGrabPose* HandGrabPose) const
{
  ...
  const auto DepthPriority = ESceneDepthPriorityGroup::SDPG_Foreground;
  const FTransform RelativeTransform = HandGrabPose->GetRelativeTransform();
  const FTransform ActorTransform = SelectedParentActor->GetActorTransform();
  for (uint8 i = 0; i < 5; i++)
  {
    ...
    BoneTransform *= RelativeTransform;
    BoneTransform *= ActorTransform;
    ...
    NextBoneTransform *= RelativeTransform;
    NextBoneTransform *= ActorTransform;
  ...
  if (VisProxy && VisProxy->Component.IsValid())
  {
    if (VisProxy->IsA(HBoneVisProxy::StaticGetType()))
    {
      ...
      if (IsValid(SelectedHandGrabPose) && SelectedHandGrabPose->IsDebugVisualActive())
      {
        ...
        CachedSelectedBoneTransform.SetScale3D(FVector::OneVector);
    ...
    FVector& DeltaScale)
{
  ...
  // Hand pose editing only supports bone rotation. Consume all input when a bone is selected
  // to prevent translate/scale from moving the parent actor.
