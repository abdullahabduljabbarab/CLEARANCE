    ...
    const FIsdkTargetTransform& Target)
{
  ...
  const FVector TargetRelativePosition = LocalPosition + InitialTargetOffset;
  const FVector ConstrainedRelativePosition =
      FIsdkTransformerUtils::GetConstrainedTransformPosition(
...
{
  for (const FIsdkGrabPose& GrabPose : GrabPoses)
  {
...
void UIsdkOneGrabTranslateTransformer::DrawDebug(const FIsdkTargetTransform& Target) const
{
  ...
  {
    const FVector GrabLocation = CachedSelectedPose.Position();
    const FRotator GrabRotation = CachedSelectedPose.Orientation().Rotator();
    const FVector TargetLocation = Target.WorldTransform.GetLocation();
    constexpr ESceneDepthPriorityGroup Depth = ESceneDepthPriorityGroup::SDPG_Foreground;
