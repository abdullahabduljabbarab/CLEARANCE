...
USceneComponent* UIsdkControllerVisualsRigComponent::GetSyntheticVisual() const
{
...
void UIsdkControllerVisualsRigComponent::GetInteractorSocket(
    ...
    FName& OutSocketName,
    USceneComponent*& OutFollowSocketComponent,
    EIsdkHandBones HandBone,
    EIsdkInteractionType InteractionType) const
{
  const EControllerHandBehavior ControllerHandBehavior =
      UIsdkFunctionLibrary::GetControllerHandBehavior(GetWorld());
  ...
  if (bUseHandAttachment)
  {
    ...
    {
      /* PokeInteractor needs to be attached to the pointer pose on controller hands to enable poke
       * limiting functionality, so we'll setup a component/socket combo for it to follow instead */
      if (InteractionType == EIsdkInteractionType::Poke)
      {
        OutSocketComponent = ControllerMeshComponent->GetPointerPoseComponent();
        OutFollowSocketComponent = AnimatedHandMeshComponent;
      }
      else
      {
    ...
    else
    {
      ...
      {
        OutSocketComponent = ControllerMeshComponent->GetPointerPoseComponent();
        OutFollowSocketComponent = PoseableHandMeshComponent;
      }
      else
      {
  ...
  {
    OutSocketComponent = ControllerMeshComponent->GetPointerPoseComponent();
    OutFollowSocketComponent = nullptr;
  }
}
...
void UIsdkControllerVisualsRigComponent::OnDataSourcesCreated()
{
  ...
  ControllerMeshComponent->AttachToComponent(
      AttachedToMotionController->GetAttachParentActor()->GetRootComponent(),
      FAttachmentTransformRules::KeepRelativeTransform);
  ControllerMeshComponent->SetMotionController(AttachedToMotionController);
  ...
  const FTransform Transform = Handedness == EIsdkHandedness::Left
...
{
  UQuestControllerAnimInstance* const ControllerAnimInstance = ControllerMeshComponent
      ? Cast<UQuestControllerAnimInstance>(ControllerMeshComponent->GetAnimInstance())
      : nullptr;
  UQuestControllerAnimInstance* const HandAnimInstance = AnimatedHandMeshComponent
  ...
  EnhancedInputComponent->BindActionInstanceLambda(
      ...
      [ControllerAnimInstance, HandAnimInstance, Lambda](const FInputActionInstance& ActionInstance)
      {
...
{
  UQuestControllerAnimInstance* const ControllerAnimInstance =
      Cast<UQuestControllerAnimInstance>(ControllerMeshComponent->GetAnimInstance());
  UQuestControllerAnimInstance* const HandAnimInstance =
      Cast<UQuestControllerAnimInstance>(AnimatedHandMeshComponent->GetAnimInstance());
...
void UIsdkControllerVisualsRigComponent::SetFollowSocket(
    USceneComponent* FollowComponentIn,
    FName SocketNameIn)
{
  ...
  ControllerMeshComponent->SetFollowBehavior(FollowComponentIn, SocketNameIn);
}
