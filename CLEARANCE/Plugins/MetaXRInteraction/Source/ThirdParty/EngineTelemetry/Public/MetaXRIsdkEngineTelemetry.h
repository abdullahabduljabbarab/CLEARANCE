...
class METAXRISDKENGINETELEMETRY_API FMetaXRIsdkEngineTelemetryModule : public IModuleInterface
{
 public:
  ...
  bool GetTelemetryWrapperReady() const
  {
 ...
 private:
  ...
  // Tracks whether we skipped Init() because another module already initialized the DLL.
  // When true, ShutdownModule() will also skip calling ShutDown() to avoid corrupting
  // the owning module's DLL state.
  static bool bSkippedInit;
};
