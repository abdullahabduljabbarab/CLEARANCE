...
{
  // Guard against calling Super::Tick when the subsystem was created outside of
  // FSubsystemCollection (e.g. via NewObject in tests). Super::Tick asserts IsInitialized(),
  // which requires the subsystem to have been initialized through a FSubsystemCollectionBase.
  // In UE 5.7, FSubsystemCollection::Initialize crashes with UMassEntityEditorSubsystem on a bare
  // UWorld, so tests must create the subsystem via NewObject instead.
  if (IsInitialized())
  {
...
UIsdkInteractorComponent* UIsdkWorldSubsystem::LookupInteractorFromPayload(
    const TWeakObjectPtr<UIsdkWorldSubsystem>& InThis,
