...
class OCULUSINTERACTIONPREBUILTS_API UIsdkTrackedDataSourceRigComponent : public USceneComponent
{
 ...
 public:
  ...
  virtual void GetInteractorSocket(
      ...
      EIsdkHandBones HandBone = EIsdkHandBones::Invalid,
      EIsdkInteractionType InteractionType = EIsdkInteractionType::Unknown) const
  {
  }

  /**
   * @brief Returns the device type (Controller, Hands, etc)
   */
  virtual EIsdkDeviceType GetDeviceType() const
  {
    return EIsdkDeviceType::Unknown;
  }

  /**
   * @brief Sets the Follow Socket for this Visual Component, to allow interactor sockets to use the
   * world transforms of component/socket combinations
   */
  virtual void SetFollowSocket(USceneComponent* FollowComponentIn, FName SocketNameIn) {}
