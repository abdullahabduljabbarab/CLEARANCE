...
bool FIsdkTestConditionalComponentIsActiveSpawnComponents::Update()
{
  ...
  const auto* const TestActor = TestWorld->SpawnActor<AIsdkTestConditionalComponentIsActive_Actor>(
      AIsdkTestConditionalComponentIsActive_Actor::StaticClass(), ActorParameters);

  if (!TestActor)
  {
    Test->AddError(TEXT("Failed to find TestActor in the scene."));
  }
...
{
  const auto* const Actor = AIsdkTestConditionalComponentIsActive_Actor::Get();
...
{
  const auto* const TestActor = AIsdkTestConditionalComponentIsActive_Actor::Get();
