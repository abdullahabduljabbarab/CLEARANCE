...
void UIsdkDataSourcesMetaXRSubsystem::Tick(float DeltaTime)
{
  ...
  // Update cached value indicating whether a controller is held by checking interaction profiles
  const bool bIsLeftHandInteractionProfile =
      UIsdkFunctionLibrary::IsHandInteractionProfile(EControllerHand::Left);
  const bool bIsRightHandInteractionProfile =
      UIsdkFunctionLibrary::IsHandInteractionProfile(EControllerHand::Right);

  // A controller is held if either hand is NOT using the hand interaction profile
  bIsHoldingAController = !bIsLeftHandInteractionProfile || !bIsRightHandInteractionProfile;
}
