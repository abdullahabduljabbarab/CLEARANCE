...
void UClearanceScenarioRunner::Start()
{
	...
	// Pause the free-play auto-spawner so random civilian traffic doesn't pollute
	// the scripted run. Suspend level-placed restricted + violation zone checks so
	// scenario aircraft aren't penalised for hitting geometry the JSON didn't
	// author. Remember the previous states so Stop() can restore them. - TripleA
	if (Controller)
	{
...
void UClearanceScenarioRunner::Stop()
{
	...
	// Restore the auto-spawner + zone checks so free-play resumes after the scenario ends.
	if (Controller)
	{
		Controller->SetAutoSpawn(bPrevAutoSpawn);
...
void UClearanceScenarioRunner::ExecuteAction(const FScenarioAction& Act)
{
	...
	case EScenarioActionType::InjectMessage:
	{
		...
		// Route through any placed VoiceOutput so the message is spoken, not just
		// drawn. Default callsign "AWACS" + a controller-style voice; the scenario
		// author can override via params. - TripleA
		...
		{
			for (TActorIterator<AClearanceVoiceOutput> VIt(Controller->GetWorld()); VIt; ++VIt)
			{
				if (*VIt) { VIt->Speak(SpeakerCs, *M, VoiceTag); break; }
			}
		}
		break;
	}
