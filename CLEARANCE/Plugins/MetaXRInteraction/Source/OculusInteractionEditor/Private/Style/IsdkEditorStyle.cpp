...
{
  TSharedRef<FSlateStyleSet> Style = MakeShareable(new FSlateStyleSet(TEXT("IsdkEditorStyle")));
  Style->SetContentRoot(
      IPluginManager::Get().FindPlugin(TEXT("OculusInteraction"))->GetBaseDir() /
      TEXT("Resources"));

  Style->Set(
      TEXT("IsdkEditor.MetaLogo"), new IMAGE_BRUSH(TEXT("ButtonIcon_80x"), FVector2D(80.f, 80.f)));
