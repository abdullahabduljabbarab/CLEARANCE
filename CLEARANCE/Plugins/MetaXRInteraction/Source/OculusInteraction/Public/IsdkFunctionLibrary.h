...
class OCULUSINTERACTION_API UIsdkFunctionLibrary final : public UBlueprintFunctionLibrary
{
 ...
 public:
  ...
  /**
   * @brief Checks if the current XR interaction profile indicates hand tracking is active.
   *
   * Uses UHeadMountedDisplayFunctionLibrary::GetCurrentInteractionProfile to query
   * the OpenXR runtime and determines if the interaction profile matches the hand
   * interaction extension profile ("/interaction_profiles/ext/hand_interaction_ext").
   *
   * @param ControllerHand The hand to check (Left or Right)
   * @return true if hand tracking is the active interaction profile, false otherwise
   */
  static bool IsHandInteractionProfile(EControllerHand ControllerHand);
};
