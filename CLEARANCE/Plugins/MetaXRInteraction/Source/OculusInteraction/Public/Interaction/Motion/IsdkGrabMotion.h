...
/**
 * @brief UIsdkGrabMotion is an abstract base class that defines how grabbed objects move
 * relative to the grabber during an active grab interaction.
 *
 * This class serves as the foundation for implementing custom grab motion behaviors in the
 * Interaction SDK. When a user grabs an object using a UIsdkGrabberComponent, the associated
 * grab motion determines how the grabbed object's transform is updated each frame. Subclasses
 * implement specific motion behaviors such as pulling objects to the hand, maintaining relative
 * position to the pointer, or manipulating objects in place.
 *
 * Use this class when you need to create a custom grab motion behavior that isn't covered by
 * the built-in motion types. To implement a custom motion, override the Start(), Stop(), and
 * Tick() methods to define your motion logic.
 *
 * The motion lifecycle is managed by UIsdkGrabberComponent:
 * - Start() is called when a grab begins (on select)
 * - Tick() is called each frame while the grab is active
 * - Stop() is called when the grab ends (on unselect)
 *
 * @see UIsdkPullToHandGrabMotion For pulling grabbed objects toward the hand position.
 * @see UIsdkRelativeToPointerGrabMotion For maintaining relative position to the pointer.
 * @see UIsdkManipulateInPlaceGrabMotion For applying grabber motion directly to the grabbable.
 * @see UIsdkGrabberComponent For the component that manages grab interactions.
 * @see UIsdkGrabbableComponent For the component that makes objects grabbable.
 */
...
class OCULUSINTERACTION_API UIsdkPullToHandGrabMotion : public UIsdkGrabMotion
{
 ...
 public:
  ...
  FORCEINLINE bool IsUsingAbsoluteTime() const
  {
    return bUseAbsoluteTime;
  }
  FORCEINLINE float GetPullSpeed() const
  {
    return PullSpeed;
  }
  FORCEINLINE float GetPullTime() const
  {
    return PullTime;
  }
  FORCEINLINE const FTransform& GetStartTransform() const
  {
