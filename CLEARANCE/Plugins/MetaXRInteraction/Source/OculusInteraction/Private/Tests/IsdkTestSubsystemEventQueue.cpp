...
bool FIsdkSubsystemTickTest::RunTest(const FString& Parameters)
{
  ...
  // Create a world and the subsystem directly via NewObject, using the world as the outer.
  // This avoids FSubsystemCollection::Initialize which initializes ALL UWorldSubsystem subclasses,
  // including UMassEntityEditorSubsystem which crashes with a bare UWorld in UE 5.7.
  UWorld* World = UWorld::CreateWorld(EWorldType::Game, false);
  UIsdkWorldSubsystem* Subsystem = NewObject<UIsdkWorldSubsystem>(World);
  ...
  // Listen to interactor/able events and forward to our test fixture
  ...
  // Invoke the interactor and drain event queue via subsystem tick
