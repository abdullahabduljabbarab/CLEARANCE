...
UIsdkHandMeshComponent::UIsdkHandMeshComponent()
{
  ...
  PoseOverride =
      CreateDefaultSubobject<UIsdkConditionalPoseOverride>(TEXT("ConditionalPoseOverride"));
...
void UIsdkHandMeshComponent::UpdateSkeleton()
{
  ...
  {
    FTransform RootPose = IIsdkIRootPose::Execute_GetRootPose(RootPoseDataSource.GetObject());
    ...
    {
      FTransform OverridePose;
      if (PoseOverride->GetRootOverride(OverridePose))
      {
        RootPose = OverridePose;
      }
      LastUpdatedRootPose = RootPose;
