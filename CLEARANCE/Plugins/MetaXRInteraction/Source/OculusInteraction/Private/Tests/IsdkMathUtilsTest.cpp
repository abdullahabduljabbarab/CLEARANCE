...
bool FIsdkMathUtilsTest::RunTest(const FString& Parameters)
{
  ...
  // Test VelocityFromSamples zero-time-delta guard. When the two samples share
  // a timestamp, the function returns ZeroVector to avoid division by zero —
  // this branch was previously untested.
  {
    FVector Velocity =
        FIsdkMathUtils::VelocityFromSamples(FVector(1, 2, 3), 1.0f, FVector(4, 5, 6), 1.0f);
    TestEqual(
        TEXT("VelocityFromSamples zero time delta returns ZeroVector"),
        Velocity,
        FVector::ZeroVector);
  }
  ...
  // Test Blueprint wrapper UIsdkMathUtilsWrapper::ChangeValueAtRate. The
  // wrapper exists solely to expose FIsdkMathUtils::ChangeValueAtRate to BP
  // and currently has no coverage; this asserts forwarding parity.
  {
    const float Result = UIsdkMathUtilsWrapper::ChangeValueAtRate(
        /*From=*/0.0f, /*To=*/10.0f, /*DeltaPerSecond=*/5.0f, /*Dt=*/1.0f);
    TestEqual(
        TEXT("UIsdkMathUtilsWrapper::ChangeValueAtRate forwards to FIsdkMathUtils"), Result, 5.0f);
  }
  return true;
}
