 ...
 * @brief Forward declarations for hand pose detection and visualization components.
 * @{
 */
 ...
 * @brief Groups hand pose data by handedness for efficient caching and lookup.
 *
 * This structure maps each handedness type (left or right) to its corresponding hand pose data.
 * It is used internally by the hand pose subsystem to store pose variations that can be applied
 * to either hand, enabling automatic mirroring and handedness-specific pose selection during
 * grab interactions.
 *
 * @see UIsdkHandPoseData For the actual pose data stored in this group.
 * @see UIsdkHandPoseSubsystem For how this is used in pose caching.
 */
USTRUCT()
struct FIsdkHandPoseDataGroup
{
  ...
  /**
   * @brief Map of handedness to hand pose data for this pose group.
   *
   * Contains pose data for one or both hands, allowing the subsystem to select the appropriate
   * pose based on which hand is interacting with an object. When automatic mirroring is enabled,
   * both left and right hand poses may be present.
   */
  UPROPERTY()
...
class OCULUSINTERACTION_API UIsdkHandPoseSubsystem : public UTickableWorldSubsystem
{
 ...
 public:
  ...
  bool SetHandPoseDetectionProfileFromMesh(
      ...
      const TArray<EIsdkFingerType>& FingersToIgnore,
      bool bIgnoreThumb,
      FName PoseName);
  ...
  /* @brief Registers a hand pose detection profile for an array of hand mesh components (typically
  ...
  UFUNCTION(BlueprintCallable, Category = InteractionSDK)
  bool RegisterHandPoseDetectionMultiple(
      UIsdkHandPoseDetectionProfile* ProfileIn,
      const TArray<UIsdkHandMeshComponent*>& HandMeshComponentsIn);
  ...
  USkinnedAsset* GetHandMesh(const EIsdkHandedness& Handedness);
  UMaterial* GetHandMeshMaterial();

 private:
  FName GetActorClassNameFromComponent(USceneComponent* ComponentIn) const;
  ...
  uint32 AddNewVariationToActorCacheMap(
      ...
      const FIsdkHandPoseDataGroup& PoseDataGroupIn,
      const FName& PoseNameIn,
      const FIsdkHandGrabPoseProperties& PosePropertiesIn);
