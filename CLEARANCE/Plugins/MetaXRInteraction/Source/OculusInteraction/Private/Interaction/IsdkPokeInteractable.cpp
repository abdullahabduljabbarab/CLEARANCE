    ...
    FActorComponentTickFunction* ThisTickFunction)
{
  ...
  const auto* HasDebugSegments = Cast<IIsdkHasDebugSegments>(SurfacePatch.GetObject());
  if (!HasDebugSegments)
  {
    return;
  }
  ...
  {
    DrawDebugLine(World, DebugSegment.Key, DebugSegment.Value, DebugColor);
    UE_VLOG_SEGMENT(
        GetOwner(),
...
{
  UIsdkInteractorComponent* EventInteractor =
      Cast<UIsdkInteractorComponent>(PointerEvent.Interactor);
  if (EventInteractor != nullptr)
  {
    EventInteractor->BroadcastPointerEvent(PointerEvent);
  }
  ...
  const FVector DebugLocation = FVector(PointerEvent.Pose.Position);

  const UIsdkRuntimeSettings* RuntimeSettings = GetDefault<UIsdkRuntimeSettings>();
  const float DebugRadius = RuntimeSettings->PointerEventDebugRadius;
  const float DebugDuration = RuntimeSettings->PointerEventDebugDuration;
