...
UIsdkRoundedButtonComponent::UIsdkRoundedButtonComponent()
{
  ...
  BackplaneMesh->bAlwaysCreatePhysicsState = false;
  ...
  LabelSize = 2.1;
  LabelText = INVTEXT("Button");
  LabelColor = FLinearColor(1.0, 1.0, 1.0, 1.0);

  static ConstructorHelpers::FObjectFinder<UCurveFloat> TransitionCurveFinder(
      IsdkContentAssetPaths::Models::RoundedButton::DefaultTransitionCurve);
  TransitionCurve = TransitionCurveFinder.Object;
}
...
{
  Super::BeginPlay();
  // Defer attachment of component's subcomponents to begin play to fix
  // inheritance template ensures
  FAttachmentTransformRules rules(
      EAttachmentRule::KeepRelative,
      EAttachmentRule::KeepRelative,
      EAttachmentRule::KeepRelative,
      /*bWeldSimulatedBodies*/ false);
  PokeButtonVisual->AttachToComponent(this, rules);
  PokeInteractable->AttachToComponent(this, rules);
  PointablePlane->AttachToComponent(this, rules);
  ButtonMesh->AttachToComponent(PokeButtonVisual, rules);
  BackplaneMesh->AttachToComponent(this, rules);
  InteractionButtonPress->AttachToComponent(this, rules);
  InteractionButtonRelease->AttachToComponent(this, rules);
  ButtonLabel->AttachToComponent(PokeButtonVisual, rules);
...
void UIsdkRoundedButtonComponent::UpdateProperties()
{
  ...
  VisualNormalColorState.Color = NormalColor;
  VisualNormalColorState.TransitionCurve = TransitionCurve;
  VisualNormalColorState.TransitionDuration = 0.1;

  auto VisualHoverColorState = FIsdkColorState();
  VisualHoverColorState.Color = HoverColor;
  VisualHoverColorState.TransitionCurve = TransitionCurve;
  VisualHoverColorState.TransitionDuration = 0.1;

  auto VisualSelectColorState = FIsdkColorState();
  VisualSelectColorState.Color = PressColor;
  VisualSelectColorState.TransitionCurve = TransitionCurve;
  VisualSelectColorState.TransitionDuration = 0.05;

  auto VisualDisabledColorState = FIsdkColorState();
  VisualDisabledColorState.Color = DisabledColor;
  VisualDisabledColorState.TransitionCurve = TransitionCurve;
