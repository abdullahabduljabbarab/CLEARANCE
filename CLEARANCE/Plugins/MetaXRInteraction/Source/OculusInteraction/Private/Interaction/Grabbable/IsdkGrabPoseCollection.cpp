...
{
  const int32 NumPoses = InPoses.Num();
...
void FIsdkGrabPoseCollection::Hover(const FIsdkGrabPose& Pose)
{
  ...
  {
    HoverPoses.Emplace(Pose);
  }
}

void FIsdkGrabPoseCollection::Unhover(int Identifier)
{
  const int PoseIndex = FindPose(HoverPoses, Identifier);
...
void FIsdkGrabPoseCollection::Select(const FIsdkGrabPose& Pose)
{
  ...
  {
    SelectPoses.Emplace(Pose);
  }
}

void FIsdkGrabPoseCollection::Unselect(int Identifier)
{
  const int SelectIndex = FindPose(SelectPoses, Identifier);
...
{
  const int PoseIndex = FindPose(HoverPoses, GrabPose.Identifier);
  if (PoseIndex != INDEX_NONE)
  {
    HoverPoses[PoseIndex].Pose = GrabPose.Pose;
  }

  const int SelectIndex = FindPose(SelectPoses, GrabPose.Identifier);
  if (SelectIndex != INDEX_NONE)
  {
