...
UIsdkGrabbableComponent* UIsdkHandGrabDetector::GetGrabCandidate(
    ...
    const UPrimitiveComponent* GrabCollider) const
{
  ...
  if (const auto ColliderAsSphere = Cast<USphereComponent>(GrabCollider))
  {
    ...
    const FVector SphereLocation = ColliderAsSphere->GetComponentLocation();
    const float SphereRadius = ColliderAsSphere->GetScaledSphereRadius();
    DrawDebugSphere(GetWorld(), SphereLocation, SphereRadius, 12, FinalDebugColor);
    UE_VLOG_WIRESPHERE(
  ...
  else if (const auto ColliderAsBox = Cast<UBoxComponent>(GrabCollider))
  {
    ...
    DrawDebugBox(
        GetWorld(), BoxLocation, BoxExtent, ColliderAsBox->GetComponentQuat(), FinalDebugColor);
    UE_VLOG_WIREOBOX(
        GrabberComponent->GetOwner(),
        LogOculusInteraction,
        Verbose,
  ...
  else
  {
    ...
    const auto Box = FBox(BoxLocation - BoxExtent, BoxLocation + BoxExtent);
    DrawDebugBox(GetWorld(), BoxLocation, BoxExtent, FinalDebugColor);
    UE_VLOG_WIREBOX(
        GrabberComponent->GetOwner(),
        LogOculusInteraction,
        Verbose,
...
void UIsdkHandGrabDetector::ComputeGrabCandidates()
{
  ...
  const bool bPinchColliderValid = IsValid(PinchCollider);
  const bool bPalmColliderValid = IsValid(PalmGrabCollider);
  const FVector PinchColliderLocation =
      bPinchColliderValid ? PinchCollider->GetComponentLocation() : FVector::ZeroVector;
  const FVector PalmColliderLocation =
      bPalmColliderValid ? PalmGrabCollider->GetComponentLocation() : FVector::ZeroVector;
  ...
  for (const auto& HoveredGrabbable : HoveredGrabbables)
  {
    ...
    const FVector GrabbableColliderLocation = GrabbableCollider->GetComponentLocation();

    // Computer nearest pinch grabbable
    if (bIsPinchGrabbable && bPinchColliderValid)
    {
      const float PinchDistance = (PinchColliderLocation - GrabbableColliderLocation).SizeSquared();
      const bool bOverlapsPinchCollider = PinchCollider->IsOverlappingComponent(GrabbableCollider);

      if (bOverlapsPinchCollider && PinchDistance < NearestPinchDistance)
      {
    ...
    if (bIsPalmGrabbable && bPalmColliderValid)
    {
      const float PalmDistance = (PalmColliderLocation - GrabbableColliderLocation).SizeSquared();
      const bool bOverlapsPalmCollider =
          PalmGrabCollider->IsOverlappingComponent(GrabbableCollider);

      if (bOverlapsPalmCollider && PalmDistance < NearestPalmDistance)
      {
