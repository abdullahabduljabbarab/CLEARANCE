...
{
  const int32 NumPoses = SelectedPoses.Num();
    ...
    const FIsdkTargetTransform& Target)
{
  ...
  const int32 NumGrabPoses = GrabPoses.Num();
  SelectedPoses.Reserve(NumGrabPoses);
  Deltas.Reserve(NumGrabPoses);
  ...
  for (const auto& GrabPose : SelectedPoses)
  {
    ...
    Deltas.Emplace(CentroidOffset, GrabPose.Orientation());
  }

  FVector CentroidToTargetVector = Centroid - Target.WorldTransform.GetLocation();
    ...
    const FIsdkTargetTransform& Target)
{
  ...
  {
    constexpr float DebugAxisThickness = 0.2f;
    DrawTransformAxis(FTransform(Centroid), DebugAxisThickness);
...
void UIsdkGrabFreeTransformer::DrawTransformAxis(const FTransform& Pose, float Thickness) const
{
  ...
  DrawDebugLine(World, JointOrigin, XAxisOffset, XColor, false, 0.0f, Depth, Thickness);
  DrawDebugLine(World, JointOrigin, YAxisOffset, YColor, false, 0.0f, Depth, Thickness);
  DrawDebugLine(World, JointOrigin, ZAxisOffset, ZColor, false, 0.0f, Depth, Thickness);
