...
{
  const FIsdkOculusXRModuleInstance& MetaXRModule = FIsdkDataSourcesMetaXRModule::GetChecked();
    ...
    bool& IsTracked)
{
  ...
  if (IsTracked)
  {
    ...
    (void)NeckPosition; // Required by GetPose API but unused
    HmdRelativePose = FTransform(MoveTemp(DeviceOrientation), MoveTemp(DevicePosition));
  }
}
