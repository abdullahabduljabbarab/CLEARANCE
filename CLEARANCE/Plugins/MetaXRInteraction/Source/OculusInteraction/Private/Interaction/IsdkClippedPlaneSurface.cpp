 ...
 public:
      ...
      UIsdkPointablePlane* const PointablePlane)
  {
    ...
    if (BoundsClippers.Num() > 0)
    {
      ...
      for (const FIsdkBoundsClipper& Clipper : BoundsClippers)
      {
        const auto& PoseProvider = Clipper.PoseProvider;
        ...
        {
          const FTransform PlaneTransform = PointablePlane->GetComponentTransform();
        ...
        IsdkBoundsClippers.emplace_back(
            isdk_BoundsClipper{
  ...
  ClippedPlaneSurfaceImpl =
      MakePimpl<isdk::api::helper::FClippedPlaneSurfaceImpl, EPimplPtrMode::NoCopy>(
          [this]() -> ClippedPlaneSurfacePtr
          {
            ...
            {
              Instance = ClippedPlaneSurface::create(*ApiPointablePlane);
            }
...
void UIsdkClippedPlaneSurface::GetDebugSegments(TArray<TPair<FVector, FVector>>& OutSegments) const
{
  ...
  constexpr float SegmentScaleFactor = 2.f;
  constexpr float SegmentInsetOffset = 0.3f;

  PointablePlane->GetDebugSegments(OutSegments);

  const int32 NumSegments = OutSegments.Num();
  ...
  Centroid /= NumSegments;

  // 2) Scale the segments by half relative to the centroid.
  for (auto& Segment : OutSegments)
  {
    FVector KeyOffset = (Segment.Key - Centroid) / SegmentScaleFactor;
    FVector ValueOffset = (Segment.Value - Centroid) / SegmentScaleFactor;

    Segment.Key = KeyOffset + Centroid;
    Segment.Value = ValueOffset + Centroid;
    ...
    Segment.Key -= KeyOffset.GetSafeNormal() * SegmentInsetOffset;
    Segment.Value -= ValueOffset.GetSafeNormal() * SegmentInsetOffset;
  }
}
