    ...
    TScriptInterface<IIsdkITransformer> Transformer)
{
  ...
  UIsdkTransformer* const TransformerObject = Cast<UIsdkTransformer>(Transformer.GetObject());
...
void UIsdkGrabTransformerComponent::BeginTransform()
{
  ...
  const int SelectCount = GrabPoses.GetSelectPoses().Num();
  ...
  if (!bWasPhysicsCached)
  {
    ...
    const bool bIsSimulatingPhysics = TransformTarget->IsSimulatingPhysics();
    if (bIsSimulatingPhysics)
    {
      UPrimitiveComponent* TargetPrimitiveComponent = Cast<UPrimitiveComponent>(TransformTarget);
      if (IsValid(TargetPrimitiveComponent))
      {
        bHadGravity = TargetPrimitiveComponent->IsGravityEnabled();
        const FVector Velocity = TargetPrimitiveComponent->GetPhysicsLinearVelocity();
...
FTransform UIsdkGrabTransformerComponent::CalculateInteractorSnapTransform(
    const FTransform& TargetTransformIn,
    const FTransform& InteractorTransformIn)
{
...
void UIsdkGrabTransformerComponent::UpdateTransform()
{
  ...
  const TArray<FIsdkGrabPose>& SelectPoses = GrabPoses.GetSelectPoses();
  const FIsdkTargetTransform TargetTransformCached(TransformTarget);
  ...
  if (bInteractorSnapEnabled && GetNumGrabbers() == 1)
  {
    ...
    else
    {
      ...
      const FTransform TargetRelativeTransform =
          ActiveGrabTransformer->UpdateTransform(SelectPoses, TargetTransformCached);
  ...
  if (GrabPoses.GetSelectPoses().IsEmpty())
  {
    ...
    if (IsValid(TargetPrimitiveComponent))
    {
      ...
      if (bIsThrowable && IsValid(ThrowableComponent))
      {
        ...
        TargetPrimitiveComponent->SetEnableGravity(bRetainGravityWhenThrown ? bHadGravity : false);
...
void UIsdkGrabTransformerComponent::ExecuteInteractorSnap(
    const FTransform& RelativeSnapPoint,
