...
bool FIsdkThrowableTest::RunTest(const FString& Parameters)
{
  ...
  SamplePositions.Reserve(3);
  SamplePositions.Emplace(FVector(0, 0, 0), 0.0f);
  SamplePositions.Emplace(FVector(2, 0, 0), 1.0f);
  SamplePositions.Emplace(FVector(4, 0, 0), 2.0f);

  TArray<TPair<FQuat, float>> SampleRotations;
  SampleRotations.Reserve(3);
  SampleRotations.Emplace(FQuat::Identity, 0.0f);
  SampleRotations.Emplace(FQuat(1, 0, 0, 0), 1.0f);
  SampleRotations.Emplace(FQuat(1, 1, 0, 0), 2.0f);
  ...
  TestEqual(TEXT("Release Velocity X"), ReleaseVelocity.X, 2.0, 0.001);
  TestEqual(TEXT("Release Velocity Y"), ReleaseVelocity.Y, 0.0, 0.001);
  TestEqual(TEXT("Release Velocity Z"), ReleaseVelocity.Z, 0.0, 0.001);
  ...
  TestEqual(TEXT("Release Angular Velocity Z"), ReleaseAngularVelocity.W, 1.0, 0.001);
