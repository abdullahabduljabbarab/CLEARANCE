    ...
    EIsdkFingerJoint Joint)
{
  const auto FoundMapping = HandJointMappings->FingerJointMappings.FindByPredicate(
      [Finger, Joint](const auto& FingerJointMapping)
    ...
    EIsdkThumbJoint Joint)
{
  const auto FoundMapping = HandJointMappings->ThumbJointMappings.FindByPredicate(
      [Joint](const auto& ThumbJointMapping) { return ThumbJointMapping.Joint == Joint; });
  return FoundMapping ? FoundMapping->Index : INDEX_NONE;
}
...
UIsdkGrabbableComponent* UIsdkFunctionLibrary::FindGrabbableByComponent(USceneComponent* Component)
{
  ...
  UIsdkGrabbableComponent* OutGrabbable = Cast<UIsdkGrabbableComponent>(Component->GetOuter());
  ...
  for (UIsdkGrabbableComponent* const Grabbable : Grabbables)
  {
...
bool UIsdkFunctionLibrary::IsHandInteractionProfile(EControllerHand ControllerHand)
{
  FString InteractionProfile;
  const bool bGotInteractionProfile =
      UHeadMountedDisplayFunctionLibrary::GetCurrentInteractionProfile(
          ControllerHand, InteractionProfile);
  return bGotInteractionProfile &&
      InteractionProfile == TEXT("/interaction_profiles/ext/hand_interaction_ext");
}
