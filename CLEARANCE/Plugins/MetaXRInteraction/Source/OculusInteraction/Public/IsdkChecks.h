...
class OCULUSINTERACTION_API UIsdkChecks : public UObject
{
 ...
 public:
      ...
      const AActor* ParentActor)
  {
    if (Dependency == nullptr)
    {
      const FString ErrorDetail = AssembleErrorDetailString(
          OnObject, ParentActor, DependencyFieldName, CallingFunction, TEXT("NULL POINTER!"));
      ...
      const AActor* ParentActor)
  {
    ...
    {
      const FString ErrorDetail = AssembleErrorDetailString(
          OnObject, ParentActor, DependencyFieldName, CallingFunction, TEXT("INVALID UOBJECT!"));
      UE_LOG(LogOculusInteraction, Error, TEXT("%s"), *ErrorDetail);
      return false;
    }
      ...
      const AActor* ParentActor)
  {
    ...
    {
      const FString ErrorDetail = AssembleErrorDetailString(
          OnObject, ParentActor, ConditionContext, CallingFunction, TEXT("CONDITION FALSE!"));
      UE_LOG(LogOculusInteraction, Error, TEXT("%s"), *ErrorDetail);
      return false;
    }
 ...
 private:
      ...
      const FString ErrorDetail)
  {
    FString DetailString =
        TEXT("[ISDK] ") + ErrorDetail + TEXT(" Context: ") + FString(ErrorContext);

    if (OnObject != nullptr)
    {
      DetailString += TEXT(" | On Object: ") + OnObject->GetFullName();
    }
    if (ParentActor != nullptr)
    {
      DetailString += TEXT(" | Parent Actor: ") + ParentActor->GetFullName();
    }
    DetailString += TEXT(" | Calling Function : ") + FString(CallingFunction);
    return DetailString;
  }
};
