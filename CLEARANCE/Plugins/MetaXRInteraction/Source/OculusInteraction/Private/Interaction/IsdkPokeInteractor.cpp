...
void UIsdkPokeInteractor::SetConfig(const FIsdkPokeInteractor_Config& InConfig)
{
  ...
  {
    auto* const Instance = PokeInteractorImpl->GetOrCreateInstance();
    isdk_PokeInteractor_Config ApiConfig;
    StructTypesUtils::Copy(Config, ApiConfig);
...
{
  auto* const Instance = GetApiPokeInteractor();
  if (Instance != nullptr)
  {
...
void UIsdkPokeInteractor::DrawDebugVisuals() const
{
  ...
  const FColor DebugColor = UIsdkDebugUtils::GetInteractorStateDebugColor(InteractorState);

  const FVector Location = GetComponentLocation();
  const float Radius = Config.Radius;

  DrawDebugSphere(GetWorld(), Location, Radius, 12, DebugColor);
  UE_VLOG_WIRESPHERE(
      GetOwner(), LogOculusInteraction, Verbose, Location, Radius, DebugColor, TEXT_EMPTY);
}
