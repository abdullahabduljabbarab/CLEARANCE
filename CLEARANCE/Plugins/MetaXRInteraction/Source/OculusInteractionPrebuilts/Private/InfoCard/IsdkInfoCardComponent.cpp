...
{
  static const FName DimensionsParamName(TEXT("Dimensions"));
  static const FName RadiiParamName(TEXT("Radii"));
  static const FName BackColorParamName(TEXT("BackColor"));
  ...
  const auto DesiredSize = _Widget ? _Widget->GetDesiredSize() : DrawSize;

  DynamicMaterial->SetVectorParameterValue(
      DimensionsParamName, FVector4(DesiredSize.X, DesiredSize.Y, 0.0, 0.0));

  DynamicMaterial->SetVectorParameterValue(
      RadiiParamName, FVector4(CornerRadius, CornerRadius, CornerRadius, CornerRadius));
  SetBackgroundColor(BackgroundColor);
  UpdateMaterialInstanceParameters();
  DynamicMaterial->SetVectorParameterValue(BackColorParamName, BackgroundColor);
}
