...
void UIsdkRayInteractor::DrawDebugVisuals(const FTransform& PointerPose) const
{
  ...
  // Named constants for debug visualization
  constexpr float DefaultInfiniteRayDistance = 10000.f;
  constexpr float SelectedThickness = 0.3f;
  constexpr float DefaultThickness = 0.2f;
  constexpr float DebugLineThickness = 0.3f;

  const EIsdkInteractorState InteractorState = GetInteractorState();
  ...
  // ray.
  float RayDistance = CollisionInfo.Value.Distance;
  const auto Instance = RayInteractorImpl->GetOrCreateInstance();
  ...
  if (RayDistance == 0.f)
  {
    RayDistance = DefaultInfiniteRayDistance;
  }

  const float Thickness =
      InteractorState == EIsdkInteractorState::Select ? SelectedThickness : DefaultThickness;
  const FVector RayStart = PointerPose.GetLocation();
  const FVector RayEnd = RayStart + RayDistance * PointerPose.GetRotation().GetForwardVector();
  DrawDebugLine(GetWorld(), RayStart, RayEnd, DebugColor, false, -1, 0, DebugLineThickness);
