...
void UIsdkRigComponent::BeginPlay()
{
  ...
  {
    APlayerController* const FirstLocalPlayer = UGameplayStatics::GetPlayerController(this, 0);

    AActor* OwnerActor = GetOwner();
    OwnerActor->EnableInput(FirstLocalPlayer);
    UEnhancedInputComponent* const EnhancedInputComponent =
        OwnerActor->FindComponentByClass<UEnhancedInputComponent>();
...
void UIsdkRigComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
  ...
  AActor* OwnerActor = GetOwner();
  UEnhancedInputComponent* const EnhancedInputComponent =
      Cast<UEnhancedInputComponent>(OwnerActor->InputComponent);
  if (IsValid(EnhancedInputComponent))
  {
    for (uint32 InputActionBindingHandle : InputActionBindingHandles)
    {
...
void UIsdkRigComponent::UpdateComponentDataSources()
{
  ...
  USceneComponent* InteractorFollowComponent = nullptr;
  FTransform InteractorAttachOffset;
  ...
  Visuals->GetInteractorSocket(
      ...
      InteractorAttachSocket,
      InteractorFollowComponent,
      RayInteractorSocket,
      EIsdkInteractionType::Ray);
  ...
  Visuals->GetInteractorSocket(
      ...
      InteractorAttachSocket,
      InteractorFollowComponent,
      PokeInteractorSocket,
      EIsdkInteractionType::Poke);
  if (Visuals->GetDeviceType() == EIsdkDeviceType::Controller &&
      InteractorFollowComponent != nullptr)
  {
    Visuals->SetFollowSocket(InteractorFollowComponent, InteractorAttachSocket);
  }
  PokeInteraction->BindDataSources(
      DataSources, InteractorAttachComponent, InteractorAttachSocket, InteractorFollowComponent);
...
void UIsdkRigComponent::InitializeHmdDataSource()
{
  ...
  if (!IsValid(HmdDataSource.GetObject()))
  {
    const TScriptInterface<IIsdkITrackingDataSubsystem> TrackingDataSubsystem =
        GetVisuals()->GetTrackingDataSubsystem();
    if (ensureMsgf(
