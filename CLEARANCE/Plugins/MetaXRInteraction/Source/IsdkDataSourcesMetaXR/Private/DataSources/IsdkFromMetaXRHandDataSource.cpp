    ...
    EIsdkHandedness InHandedness)
{
  ...
  if (ComponentClass)
  {
    ...
    if (FBoolProperty* UpdateHandScaleProp = CastField<FBoolProperty>(
            OculusHandComponent->GetClass()->FindPropertyByName(TEXT("bUpdateHandScale"))))
    {
      UpdateHandScaleProp->SetPropertyValue_InContainer(OculusHandComponent, true);
    }
    ...
    FActorComponentTickFunction* ThisTickFunction)
{
  ...
  // Check if the current interaction profile indicates hands are being used
  const EControllerHand ControllerHand =
      Handedness == EIsdkHandedness::Left ? EControllerHand::Left : EControllerHand::Right;
  const bool bIsHandInteractionProfile =
      UIsdkFunctionLibrary::IsHandInteractionProfile(ControllerHand);
  ...
  const bool bShouldReadHandJointData =
      bIsHandInteractionProfile || PoseType != EIsdkXRControllerDrivenHandPoseType::None;
  ...
  IsRootPoseConnected->SetValue(
      MotionController && bIsHandInteractionProfile && MotionController->IsTracked());
...
{
  return bIsLastGoodPointerPoseValid && IsRootPoseConnected->GetResolvedValue();
}
...
void UIsdkFromMetaXRHandDataSource::ReadHandData()
{
  ...
  if (CanUseHandData)
  {
    ...
    const float HandScale = IsValid(OculusXrHandComponent)
        ? OculusXrHandComponent->GetComponentTransform().GetScale3D().X
        : 1.0f;
    ...
    for (const auto& Bone : BoneMap)
    {
      ...
      Pose.SetLocation(Pose.GetLocation() * HandScale);
    ...
    const int32 NumJoints = JointRadii.Num();
    ...
    bool& IsValid)
{
  if (MotionController)
  {
    ...
    IsValid = bIsLastGoodPointerPoseValid && IsRootPoseConnected->GetResolvedValue();
  }
  else
  {
    IsValid = false;
...
void UIsdkFromMetaXRHandDataSource::GenerateBoneMap()
{
  ...
  const FVector PalmOffset = Handedness == EIsdkHandedness::Left
      ? FVector(-0.145789, 5.974506, 1.9)
      : FVector(0.311415, -6.273499, -1.9);
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedOffset(0, PalmOffset, 0.0));

  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(2, BoneNames[3])); // HandThumb1 = 2
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(3, BoneNames[4])); // HandThumb2 = 3
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(4, BoneNames[5])); // HandThumb3 = 4
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(5, BoneNames[19])); // HandThumbTip = 5

  const float RotationMult = Handedness == EIsdkHandedness::Left ? -1.0 : 1.0;

  const FVector Index0Offset = Handedness == EIsdkHandedness::Left
      ? FVector(2.004335, 3.778780, 0.284375)
      : FVector(-1.908977, -3.500033, -0.284375);
  const float IndexRotation = RotationMult * PI * 0.1;
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedOffset(6, Index0Offset, IndexRotation)); // HandIndex0 = 6
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(7, BoneNames[6])); // HandIndex1 = 7
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(8, BoneNames[7])); // HandIndex2 = 8
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(9, BoneNames[8])); // HandIndex3 = 9
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(10, BoneNames[20])); // HandIndexTip = 10

  const FVector Middle0Offset = Handedness == EIsdkHandedness::Left
      ? FVector(0.383252, 3.780457, -0.507344)
      : FVector(-0.295005, -3.552834, 0.507344);
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedOffset(11, Middle0Offset, 0.0)); // HandMiddle0 = 11
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(12, BoneNames[9])); // HandMiddle1 = 12
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(13, BoneNames[10])); // HandMiddle2 = 13
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(14, BoneNames[11])); // HandMiddle3 = 14
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(15, BoneNames[21])); // HandMiddleTip = 15

  const FVector Ring0Offset = Handedness == EIsdkHandedness::Left
      ? FVector(-1.035655, 3.796854, -0.203718)
      : FVector(0.993480, -3.424118, 0.203718);
  const float RingRotation = RotationMult * -PI * 0.1;
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedOffset(16, Ring0Offset, RingRotation)); // HandRing0 = 16
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(17, BoneNames[12])); // HandRing1 = 17
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(18, BoneNames[13])); // HandRing2 = 18
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(19, BoneNames[14])); // HandRing3 = 19
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(20, BoneNames[22])); // HandRingTip = 20

  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(21, BoneNames[15])); // HandPinky0 = 21
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(22, BoneNames[16])); // HandPinky1 = 22
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(23, BoneNames[17])); // HandPinky2 = 23
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(24, BoneNames[18])); // HandPinky3 = 24
  BoneMap.Emplace(FBoneOVRToOXRMap::MappedIndex(25, BoneNames[23])); // HandPinkyTip = 25,
}
