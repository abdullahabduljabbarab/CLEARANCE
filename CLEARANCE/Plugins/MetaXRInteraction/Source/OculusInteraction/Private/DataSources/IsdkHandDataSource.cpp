    ...
    FActorComponentTickFunction* ThisTickFunction)
{
  ...
  if (ShouldUpdateDataSource())
  {
    ...
    if (ApiInstance)
    {
      isdk_DataSourceUpdateDataResult RetVal = ApiInstance->updateData();
...
{
  if (!ensureMsgf(InDataSource, TEXT("%s must be set"), *ErrMsgFieldName))
  {
    return nullptr;
  }

  isdk::api::IHandDataSource* const ApiFromDataSource = InDataSource->GetApiIHandDataSource();
...
void UIsdkHandDataSource::ReadHandJointsFromImpl()
{
  ...
  if (ApiInstance)
  {
    ...
    int OutCount{};
    const auto DataSourceHandle = ApiInstance->getIHandDataSourceHandle();
    const int NumJoints = HandData->GetNumJoints();
    isdk_IHandDataSource_getJoints(
        DataSourceHandle, &DataSourceImpl->HandData.joints[0], NumJoints, &OutCount);
