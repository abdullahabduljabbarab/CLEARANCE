    ...
    bool bUseVolumeRangeAsOffset)
{
  if (!Sounds.IsEmpty())
  {
    const int32 SelectedIndex = FMath::RandRange(0, Sounds.Num() - 1);
    ...
    const float VolumeOffset = bUseVolumeRangeAsOffset
        ? HalfVolumeRange
...
FVector UIsdkGrabbableAudio::GetCurrentScale() const
{
  if (IsValid(GrabTransformer))
  {
    USceneComponent* Target = GrabTransformer->GetTransformTarget();
    if (IsValid(Target))
    {
...
float UIsdkGrabbableAudio::GetScaleDelta() const
{
...
void UIsdkGrabbableAudio::BeginPlay()
{
  ...
  if (!IsValid(GrabTransformer) && GrabTransformerName != NAME_None)
  {
    ...
    for (UActorComponent* const Component : Components)
    {
      ...
      {
        UIsdkGrabTransformerComponent* FoundGrabTransformer =
            Cast<UIsdkGrabTransformerComponent>(Component);
...
{
  for (USoundBase* const Sound : Sounds)
  {
...
void UIsdkGrabbableAudio::UpdateScaling()
{
  ...
  if (!ScalingSounds.IsEmpty())
  {
    const FVector CurrentScale = GetCurrentScale();
    const float PrevMagnitude = PreviousScale.Length();
    const float NewMagnitude = CurrentScale.Length();
    const float TotalDelta = NewMagnitude - PrevMagnitude;
    const float AbsTotalDelta = FMath::Abs(TotalDelta);
    ...
    if (AbsTotalDelta > StepSize)
    {
      ...
      if ((CurrentTime - LastEventTime) >= 1.f / MaxEventFreq)
      {
        ...
        const float PitchOffset = TotalDelta > 0.0f ? ScalingPitchRange : -ScalingPitchRange;
        PlaySound(ScalingSounds, ScalingBasePitch, PitchOffset, GrabbingBaseVolume, 0.0f, true);
      }
    }
  }
}
    ...
    const UIsdkGrabTransformerComponent* InGrabTransformer)
{
  switch (Event)
  {
    ...
    {
      const int32 CurrentPoseCount = GrabTransformer->GetGrabPoses().GetSelectPoses().Num();
      // Added a pose
