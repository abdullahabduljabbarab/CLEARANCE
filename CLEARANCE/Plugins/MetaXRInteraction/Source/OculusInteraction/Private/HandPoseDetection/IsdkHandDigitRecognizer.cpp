...
void UIsdkHandDigitRecognizer::UpdateState(float DeltaTime)
{
  ...
  isdk::api::IHandPositionFrame* const HandPositionFrame = EnsureHandPositionFrame();
  if (HandPositionFrame)
  {
    ApiDigitRecognizer->update(DeltaTime, HandPositionFrame->getIHandPositionFrameHandle());
  }
}
...
isdk::api::IHandPositionFrame* UIsdkHandDigitRecognizer::EnsureHandPositionFrame() const
{
  ...
  bValidInputs &= UIsdkChecks::EnsureMsgfDependencyIsValid(HandMesh, this, TEXT("HandMesh"));
  if (!bValidInputs)
  {
    return nullptr;
  }

  isdk::api::IHandPositionFrame* const ApiHandPositionFrame =
      HandMesh->TryGetApiIHandPositionFrame();
  bValidInputs &= UIsdkChecks::EnsureMsgfDependencyApiIsInitialized(
