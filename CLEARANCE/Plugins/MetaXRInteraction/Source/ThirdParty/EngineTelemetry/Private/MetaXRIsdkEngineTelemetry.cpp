...
bool FMetaXRIsdkEngineTelemetryModule::bSkippedInit = false;

void FMetaXRIsdkEngineTelemetryModule::StartupModule()
{
#if PLATFORM_WINDOWS
  // Check if another plugin has already loaded and initialized EngineTelemetry.dll.
  // Both MetaXR (OculusXRTelemetry) and OculusPlatform (OVRPlatformEngineTelemetry) ship their own
  // copy of the same DLL, but Windows LoadLibrary deduplicates by module name, so the second load
  // returns the already-loaded instance. Calling engineTelemetry_Init() a second time on that shared
  // instance can corrupt its internal state and cause crashes (e.g. in DebugGame builds where module
  // load ordering differs). If another module already initialized telemetry, we bind our function
  // pointers from the already-loaded DLL but skip the redundant Init() call.
  const bool bTelemetryAlreadyLoaded =
      FModuleManager::Get().IsModuleLoaded(TEXT("OculusXRTelemetry")) ||
      FModuleManager::Get().IsModuleLoaded(TEXT("OVRPlatformEngineTelemetry"));
  ...
  if (DllHandle)
  {
        ...
        EngineTelemetryGetNotifyText && EngineTelemetryGetSettingsChangedText)
    {
      ...
      if (bTelemetryAlreadyLoaded)
      {
        UE_LOG(
            LogIsdkEngineTelemetry,
            Log,
            TEXT("OculusXRTelemetry module already loaded - skipping redundant engineTelemetry_Init()"));
        bSkippedInit = true;
      }
      else
      {
...
{
#if PLATFORM_WINDOWS
  // Only call engineTelemetry_ShutDown() if we were the ones who called Init().
  // If MetaXR owns the DLL lifecycle, shutting down here would corrupt its state.
  if (!bSkippedInit)
  {
    ShutDownTelemetry();
  }
    ...
    const char* BatchMode)
{
  ...
  {
    const FString ProjectNameStr = FApp::GetProjectName();
