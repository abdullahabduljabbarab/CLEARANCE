...
{
  FTransform GripPose;
  IsRootPoseHighConfidence->SetValue(GetGripPose(GripPose));
  bIsLastGoodPointerPoseValid = true;
}

bool UIsdkFromOpenXRControllerDataSource::IsHandTrackingEnabled() const
{
  auto& ModularFeatures = IModularFeatures::Get();
  const FName HandTrackerFeatureName = IHandTracker::GetModularFeatureName();
  if (ModularFeatures.IsModularFeatureAvailable(HandTrackerFeatureName))
  {
    const auto& HandTracker =
        ModularFeatures.GetModularFeature<IHandTracker>(HandTrackerFeatureName);
    return HandTracker.IsHandTrackingStateValid();
  }
  return false;
}

bool UIsdkFromOpenXRControllerDataSource::GetGripPose(FTransform& OutGripPose) const
{
