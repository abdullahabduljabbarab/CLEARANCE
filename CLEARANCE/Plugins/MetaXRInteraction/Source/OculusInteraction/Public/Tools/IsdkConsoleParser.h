...
/**
 * Multicast delegate that broadcasts when a console command is received.
 *
 * @param Command The command string that was entered.
 * @param bHandled Pointer to a boolean indicating whether the command was handled.
 */
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnConsoleCommand, FString, bool*);

/**
 * Interface class for objects that need to receive and process Interaction SDK console commands.
 *
 * This interface enables a listener pattern where multiple objects can register to receive
 * console commands entered via the Unreal Engine console with the "isdk" prefix. Implement
 * this interface in any class that needs to respond to runtime debugging or configuration
 * commands. Register your implementation with UIsdkConsoleParser::RegisterListener() during
 * BeginPlay and unregister with UIsdkConsoleParser::UnregisterListener() during EndPlay.
 *
 * @see UIsdkConsoleParser For the main console command parsing and dispatch system.
 * @see UIsdkConsoleParser::RegisterListener For registering command receivers.
 */
...
/**
 * Native interface for receiving Interaction SDK console commands.
 *
 * Implement ProcessConsoleCommand() to handle commands dispatched by UIsdkConsoleParser.
 * Commands are received as an array of string arguments parsed from the console input.
 *
 * @see UIsdkConsoleReceiver For the UInterface wrapper required for Blueprint compatibility.
 */
class IIsdkConsoleReceiver
{
 ...
 public:
  /**
   * Called when a console command prefixed with "isdk" is entered.
   *
   * Override this method to handle specific console commands for debugging or runtime
   * configuration. The first element of TextArgs contains the command name (lowercase),
   * and subsequent elements contain any additional parameters.
   *
   * @param TextArgs Array of command arguments. Index 0 is the command, index 1+ are parameters.
   * @param World The current world context, may be nullptr in some testing scenarios.
   * @return True if the command was handled by this receiver, false otherwise.
   *
   * @see UIsdkConsoleParser::ParseConsoleCommand For how commands are dispatched.
   */
...
class OCULUSINTERACTION_API UIsdkConsoleParser : public UObject
{
 ...
 public:
  ...
  {
    const FString s = GetFStringValue(Keyword);
  ...
  static void ParseConsoleCommand(const TArray<FString>& TextArgs, UWorld* World)
  {
    if (!TextArgs.IsEmpty())
    {
      const FString IsdkCommand = TextArgs[0].ToLower();
      const FString param = TextArgs.Num() > 1 ? TextArgs[1] : TEXT("");
      Get()->Values.Emplace(IsdkCommand, param);
      for (IIsdkConsoleReceiver* const Receiver : Get()->Receivers)
      {
