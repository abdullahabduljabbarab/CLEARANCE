...
{
  const int32 ThumbJointMappingCount = ThumbJointMappings.Num();
  const int32 FingerJointMappingCount = FingerJointMappings.Num();
  ...
  for (int32 Index = 0; Index < ThumbJointMappingCount; ++Index)
  {
    HandJointMappings->ThumbJointMappings[Index] = ThumbJointMappings[Index];
  }
  for (int32 Index = 0; Index < FingerJointMappingCount; ++Index)
  {
...
void UIsdkExternalHandDataSource::SetImplHandData(const FTransform& WorldTransform)
{
  ...
  if (Instance != nullptr)
  {
    ...
    const isdk_DataSourceUpdateDataResult RetVal =
