...
bool IsdkRigHandPointerPoseSocketTests::RunTest(const FString& Parameters)
{
  ...
  ADD_LATENT_AUTOMATION_COMMAND(FIsdkTestRigLambda(
      ...
      {
        auto* FakeHandDataSource = Actor.GetFakeHandDataSource();
        FakeHandDataSource->FakeRootPose = TestRootPose;
        FakeHandDataSource->bIsFakePointerPoseValid = true;
        FakeHandDataSource->FakePointerPoseRelative = TestRelativeTransform;
      }));
  ADD_LATENT_AUTOMATION_COMMAND(FIsdkTestRigLambda(
