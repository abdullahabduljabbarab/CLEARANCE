...
AIsdkInformationFrame::AIsdkInformationFrame()
{
  ...
  FloorOutline = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("FloorOutline"));
  FloorOutline->SetupAttachment(InformationFrameRoot);

  FloorOutline->SetCollisionEnabled(ECollisionEnabled::NoCollision);

  InfoCard = CreateDefaultSubobject<UIsdkInfoCardComponent>(TEXT("InfoCardComponent"));
  InfoCard->SetupAttachment(InformationFrameRoot);
  InfoCard->SetRelativeRotation(FRotator(30, -90, 0));
  InfoCard->SetPivot(FVector2D(0.5, 1));
  InfoCard->SetRelativeScale3D(FVector(0.015f, 0.015f, 0.015f));
  InfoCard->SetLabelText(LabelText);
  InfoCard->SetBodyText(BodyText);
}
...
void AIsdkInformationFrame::Initialize()
{
  ...
  FVector CardLocation = FVector(0, -OutlineSize.Y * 0.5, 0.0);
  CardLocation += FVector(CardOffset.X, CardOffset.Y, 0.0);

  InfoCard->SetRelativeLocation(CardLocation);
  InfoCard->SetLabelText(LabelText);
  InfoCard->SetBodyText(BodyText);
...
{
  const double OffsetMultiplier = Properties.Alignment == EHorizTextAligment::EHTA_Center ? 0.0
      : Properties.Alignment == EHorizTextAligment::EHTA_Left                             ? 1.0
                                                                                          : -1.0;
  const double Offset = ContainerSize.Y * 0.5 * OffsetMultiplier;
  const FVector TextSize = TextRender->GetTextLocalSize();
  FVector PrimaryRelativeLocation =
