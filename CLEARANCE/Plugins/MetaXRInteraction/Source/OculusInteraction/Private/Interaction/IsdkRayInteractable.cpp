    ...
    FActorComponentTickFunction* ThisTickFunction)
{
  ...
  auto* const HasDebugSegments = Cast<IIsdkHasDebugSegments>(Surface.GetObject());
  if (!HasDebugSegments)
  {
    return;
  }
...
void UIsdkRayInteractable::HandlePointerEvent(const FIsdkInteractionPointerEvent& PointerEvent)
{
  ...
  const FVector DebugLocation(PointerEvent.Pose.Position);
  const UIsdkRuntimeSettings* RuntimeSettings = GetDefault<UIsdkRuntimeSettings>();
  const auto DebugRadius = RuntimeSettings->PointerEventDebugRadius;
  const auto DebugDuration = RuntimeSettings->PointerEventDebugDuration;
