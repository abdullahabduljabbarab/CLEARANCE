...
bool FIsdkTestCheckInteractionRelationships::Update()
{
  ...
  // Test Interactable
  {
    ...
    UObject* const TestInteractorObject = TestInteractor.GetObject();
    ...
    if (ExpectedCounts.NumHover == 1 && RelationshipInteractors.Num() > 0)
    {
      Test->TestEqual(
          ...
          TestInteractorObject);
    }

    RelationshipInteractors.Reset();
    TestInteractable->GetInteractableStateRelationships(
  ...
  // Test Interactor
  {
    ...
    UObject* const TestInteractableObject = TestInteractable.GetObject();
    ...
    RelationshipInteractables.Reset();
    TestInteractor->GetInteractorStateRelationships(
