    ...
    FActorComponentTickFunction* ThisTickFunction)
{
  ...
  if (IsValid(InputDataSource) && IsValid(HandData))
  {
    ...
    // Copy joint data from input source to allow modification
    HandData->SetJoints(HandJointsUE->GetJointPoses());
    // Allow derived classes and Blueprint to apply custom modifications
    OnModify(HandData);
    // Propagate the modified hand data downstream with the original root pose
