...
TOptional<isdk_SyntheticHand_WristLockMode> Convert(const EIsdkRootPoseLockMode LockMode)
{
    ...
    bool bSkipAnimation)
{
  ...
  {
    auto* const ApiInstance = SyntheticHandImpl->GetOrCreateInstance();
    const auto ApiPose = StructTypesUtils::Convert(Transform);
    ApiInstance->lockWristPose(
    ...
    bool bSkipAnimation)
{
  ...
  ApiInstance->lockWristPosition(&ApiPosition, OverrideFactor, bSkipAnimation ? 1 : 0);
}
    ...
    bool bSkipAnimation)
{
  ...
  ApiInstance->lockWristRotation(&ApiRotation, OverrideFactor, bSkipAnimation ? 1 : 0);
}
...
void UIsdkSyntheticHand::FreeRootTransform_Implementation(EIsdkRootPoseLockMode LockMode)
{
  ...
  {
    auto* const ApiInstance = SyntheticHandImpl->GetOrCreateInstance();
    ApiInstance->freeWrist(WristLockMode.GetValue());
  }
}
